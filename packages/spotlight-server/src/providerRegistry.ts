...
export type SpotlightProviderConfig = Record<string, unknown> & {
  type: string;
};
...
export class SpotlightProviderRegistry {
  ...
  async createKnowledge(
    config?: SpotlightProviderConfig,
  ): Promise<KnowledgeProvider | undefined> {
  ...
  async createWebSearch(
    config?: SpotlightProviderConfig,
  ): Promise<WebSearchProvider | undefined> {
    if (!config) return undefined;
    const factory = this.web.get(config.type.trim().toLowerCase());
    if (!factory)
      throw new Error(`Unknown web search provider: ${config.type}`);
    return factory(config);
  }
...
export function adaptLangChainTool(
  tool: {
    name: string;
    description: string;
  ...
  metadata: SpotlightServerTool["metadata"],
): SpotlightServerTool {
  return {
    name: tool.name,
